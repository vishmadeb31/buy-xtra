...
export const ProductDetail: React.FC = () => {
  ...
  return (
    <div className="bg-white min-h-screen pb-24 md:pb-0 relative">
      <div className="container mx-auto px-0 md:px-4 py-0 md:py-4 max-w-7xl">
        
        {/* Breadcrumb / Back (Mobile) */}
        <div className="md:hidden p-4 flex items-center border-b sticky top-16 bg-white z-40 shadow-sm">
...
const SpecItem = ({ icon, label, value }: { icon: React.ReactNode, label: string, value?: string }) => {
  ...
  return (
    <div className="flex items-start gap-3 p-3 border border-gray-100 rounded-lg hover:shadow-sm transition-shadow">
       <div className="p-2 bg-blue-50 rounded-full text-[#2874F0] shrink-0">
          {React.cloneElement(icon as React.ReactElement<{ className?: string }>, { className: "w-5 h-5" })}
       </div>
       <div>
          <p className="text-xs text-[#878787] font-bold uppercase mb-1">{label}</p>
          <p className="text-sm text-[#212121] font-medium leading-tight">{value}</p>
       </div>
    </div>
  );
}